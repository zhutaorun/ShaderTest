Shader "Unlit/Phong"
{
	Properties
	{
		...
		_SpecCol ("_SpecColor", Color) = (1,1,1,1)
		_Glossiness ("_Glossiness", Range(8,64)) = 8
	}
	...
	half4 frag (v2f i) : SV_Target
	{
		...
		half3 reflDir = reflect(-lightDir,normal);//计算反射光线方向
		half NoL = saturate(dot(normal,lightDir));//计算法线光线点积
		half VoR = saturate(dot(viewDir,reflDir));//计算射线反射点积
		half D = pow(VoR,_Glossiness);//计算高光分布

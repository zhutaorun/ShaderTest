...
public class ColorPicker : MonoBehaviour {
	...
	void Awake(){
		...
		if (m_camera == null){
			Debug.LogError ("You need to drat this script to a camera!");
			return;
		}
		...
		if(pickerCollider==null){
			...
			pickerCollider.center += m_camera.transform.worldToLocalMatrix.MultiplyVector (m_camera.transform.transform.forward) * (m_camera.nearClipPlane + 0.2f);
	...
	//OnPostRender is called affer a camera has finished rendering the scene.
